...
--AddCSLuaFile( "other/cl_hudnew.lua" )
...
function GetActivePlayers()
	...
	for k, v in pairs(player.GetAll()) do
		...
		if v.ActivePlayer == true then table.ForceInsert(tab, v) end
	end
	return tab
end
...
function GetNotActivePlayers()
	...
	for k, v in pairs(player.GetAll()) do
		if v.ActivePlayer == nil then v.ActivePlayer = true end
		if v.ActivePlayer == false then table.ForceInsert(tab, v) end
	end
	return tab
end
...
function PlayerNTFSound(sound, ply)
	if (ply:Team() == TEAM_GUARD or ply:Team() == TEAM_CHAOS) and ply:Alive() then
		...
		--ply:EmitSound( "breach/Beep.ogg", 500, 100, 1 )
...
timer.Create("BlinkTimer", GetConVar("br_time_blinkdelay"):GetInt(), 0, function()
	...
	timer.Create("UnBlinkTimer", time + 0.2, 1, function()
		for k, v in pairs(player.GetAll()) do
			if v.blinkedby173 == false then v.isblinking = false end
		end
	end)
end)
...
function RequestOpenGateA(ply)
	...
	for id, ent in pairs(ents.FindByClass("func_button")) do
		for k, v in pairs(MAPBUTTONS) do
			if v["pos"] == ent:GetPos() then if v["name"] == "Gate A" then gatea = ent end end
		end
	end

	if IsValid(gatea) then
		nextgateaopen = CurTime() + 60
		timer.Simple(2, function() if IsValid(gatea) then gatea:Use(ply, ply, USE_ON, 1) end end)
	end
end

local lastpocketd = 0
function GetPocketPos()
	if lastpocketd > #POS_POCKETD then lastpocketd = 0 end
...
function Kanade()
	for k, v in pairs(player.GetAll()) do
		if v:SteamID64() == "76561198156389563" then return v end
	end
end
...
function GetSCPLeavers()
	...
	for k, v in pairs(team.GetPlayers(TEAM_SPECTATOR)) do
		if v.Leaver == "scp" then table.ForceInsert(tab, v) end
	end
...
function GetClassDLeavers()
	local tab = {}
	for k, v in pairs(team.GetPlayers(TEAM_SPECTATOR)) do
		if v.Leaver == "classd" then table.ForceInsert(tab, v) end
	end
...
function GetSciLeavers()
	local tab = {}
	for k, v in pairs(team.GetPlayers(TEAM_SPECTATOR)) do
		if v.Leaver == "sci" then table.ForceInsert(tab, v) end
	end
...
function SpawnNTFS()
	...
	if spawnedntfs > 100 then return end
	local allspecs = {}
	for k, v in pairs(team.GetPlayers(TEAM_SPECTATOR)) do
		if v.ActivePlayer == true then table.ForceInsert(allspecs, v) end
	end

	local num = math.Clamp(#allspecs, 0, 5)
	local spawnci = false
	spawnedntfs = spawnedntfs + num
	if math.random(1, 5) == 1 then spawnci = true end
	for i = 1, num do
...
function ForceUse(ent, on, int)
	for k, v in pairs(player.GetAll()) do
		if v:Alive() then ent:Use(v, v, on, int) end
	end
end

function OpenGateA()
	for k, v in pairs(ents.FindByClass("func_rot_button")) do
		if v:GetPos() == POS_GATEABUTTON then ForceUse(v, 1, 1) end
	end
end
...
function CloseSCPDoors()
	...
	for k, v in pairs(ents.FindByClass("func_door")) do
		-- 173 doors
		if v:GetPos() == POS_173DOORS then ForceUse(v, 0, 1) end
		-- 106 doors
		if v:GetPos() == POS_106DOORS then ForceUse(v, 0, 1) end
		-- 049 doors
		if v:GetPos() == POS_049BUTTON then ForceUse(v, 0, 1) end
		-- 096 first doors
		if v:GetPos() == POS_096DOORS1 then ForceUse(v, 0, 1) end
		-- 096 second doors
		if v:GetPos() == POS_096DOORS2 then ForceUse(v, 0, 1) end
	end

	for k, v in pairs(ents.FindByClass("func_button")) do
		if v:GetPos() == POS_173BUTTON then ForceUse(v, 1, 1) end
		if v:GetPos() == POS_049BUTTON then ForceUse(v, 1, 1) end
		if v:GetPos() == POS_096DBUTTON1 then ForceUse(v, 1, 1) end
		if v:GetPos() == POS_096DBUTTON2 then ForceUse(v, 1, 1) end
	end
end
...
function OpenSCPDoors()
	...
	for k, v in pairs(ents.FindByClass("func_door")) do
		-- 173 doors
		if v:GetPos() == POS_173DOORS then ForceUse(v, 1, 1) end
		-- 106 doors
		if v:GetPos() == POS_106DOORS then ForceUse(v, 1, 1) end
		-- 096 first doors
		if v:GetPos() == POS_096DOORS1 then ForceUse(v, 1, 1) end
		-- 096 second doors
		if v:GetPos() == POS_096DOORS2 then ForceUse(v, 1, 1) end
	end
...
function GetAlivePlayers()
	local plys = {}
	for k, v in pairs(player.GetAll()) do
		if v:Team() ~= TEAM_SPECTATOR then if v:Alive() then table.ForceInsert(plys, v) end end
	end
	return plys
end
