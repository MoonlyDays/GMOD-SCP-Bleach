...
function HeadBob(pl, pos, ang, fov)
	...
	if pl:IsOnGround() and HBEnabled then
		if pl:KeyDown(IN_FORWARD) then
			if HBAP < 1.5 then HBAP = HBAP + 0.05 * speedmul end
		else
			if HBAP > 0 then HBAP = HBAP - 0.05 * speedmul end
		end
		...
		if pl:KeyDown(IN_MOVELEFT) then
			if HBAR > -1.5 then HBAR = HBAR - 0.07 * speedmul end
		else
			if HBAR < 0 then HBAR = HBAR + 0.07 * speedmul end
		end
