...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	if ply:GetNClass() ~= ROLES.ROLE_SCP173 then CreateRagdollPL(ply, attacker, dmginfo:GetDamageType()) end
	ply:AddDeaths(1)
end

-- From Gmod base
function GM:PlayerDeathThink(ply)
	if ply:IsFrozen() == true then ply:Freeze(false) end
		...
		for k, v in pairs(victim:GetWeapons()) do
			...
			if candrop then
				...
				if IsValid(wep) then
					...
					if atype > 0 then wep.SavedAmmo = v:Clip1() end
				end
			end
		end
	end
...
function GM:PlayerCanHearPlayersVoice(listener, talker)
	...
	if talker:GTeam() == TEAM_SCP then if listener:GTeam() ~= TEAM_SCP then return false end end
...
function GM:PlayerCanSeePlayersChat(text, teamOnly, listener, talker)
	if talker.Alive == nil then return true end
	if talker:Alive() == false then return false end
	if listener:Alive() == false then return false end
	if teamOnly then
		if talker:GetPos():Distance(listener:GetPos()) < 750 then
	...
	if ply:GTeam() ~= TEAM_SPEC then
		if wep.teams then
			...
			for k, v in pairs(wep.teams) do
				if v == ply:GTeam() then canuse = true end
			end
...
function CacheButtons()
	local num_cached = 0
	local cbuttons = table.Copy(MAP_BUTTONS)
	...
	for k, v in pairs(cbuttons) do
		if v.found_entity == false then print("Button was not cached! " .. v.name) end
	end

	print("cached " .. num_cached .. "/" .. #MAP_BUTTONS .. " buttons")
end
...
function ShouldPlayerUse(ply, ent)
	if istable(ent.BreachButton) then
		if ent.BreachButton.canactivate ~= nil then
			...
			if canactivate then
				if ent.BreachButton.usesounds == true then ply:EmitSound("KeycardUse1.ogg") end
				ply.lastuse = CurTime() + 1
				ply:PrintMessage(HUD_PRINTCENTER, "Access granted to " .. ent.BreachButton["name"])
				return true
			else
				if ent.BreachButton.usesounds == true then ply:EmitSound("KeycardUse2.ogg") end
				ply.lastuse = CurTime() + 1
...
function BreachTick()
	if isfunction(MAP_THINK) then MAP_THINK() end
	for k, v in pairs(player.GetAll()) do

		if v.GTeam() == TEAM_SPEC then
			local target = v:GetObserverTarget()
			if IsValid(target) and target:IsPlayer() then if target:GTeam() == TEAM_SPEC then v:SpectatePlayerLeft() end end
		end

		if v.nextDamageInPD == nil then v.nextDamageInPD = 0 end
		if v.nextDamageInGas == nil then v.nextDamageInGas = 0 end
		if v.LastCough == nil then v.LastCough = 0 end
		if v.NextCough == nil then v.NextCough = 0 end
		if v:Alive() == true and v:GTeam() ~= TEAM_SPEC and v:GTeam() ~= TEAM_SCP then
			...
			if InGas(v) == true and v.nextDamageInGas < CurTime() then
				...
				if v.NextCough < CurTime() then
					...
					if v.LastCough > 3 then v.LastCough = 1 end
					print(v.LastCough)
