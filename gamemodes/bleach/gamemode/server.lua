...
--util.AddNetworkString("173HLentities")
...
net.Receive("DropCurrentVest", function(len, ply) if ply:GTeam() ~= TEAM_SPEC and ply:GTeam() ~= TEAM_SCP and ply:Alive() then if ply.UsingArmor ~= nil then ply:UnUseArmor() end end end)
...
net.Receive("DropWeapon", function(len, ply)
	...
	if IsValid(wep) and wep ~= nil and IsValid(ply) then
		...
		if atype > 0 then wep.SavedAmmo = wep:Clip1() end
...
function IsRoleMaxed(role)
	...
	for _, pl in pairs(player.GetAll()) do
		if pl:GetNClass() == role.name then using = using + 1 end
	end
...
function uber_random_test(num, tests)
	...
	for i = 1, tests do
		...
		if tab[rnd] == nil then tab[rnd] = 0 end
		tab[rnd] = tab[rnd] + 1
	end
	...
	for k, v in pairs(players_to_use) do
		if our_role_list[ROLE_LIST_SECURITY] ~= nil and our_role_list[ROLE_LIST_SECURITY] > 0 then
			...
			for k2, role in pairs(security_roles) do
				local can = true
				if isfunction(role.customcheck) then if role.customcheck(v) == false then can = false end end
				if can == true then
		...
		elseif our_role_list[ROLE_LIST_SCPS] ~= nil and our_role_list[ROLE_LIST_SCPS] > 0 then
			...
			if #spc_tab < 1 then spc_tab = table.Copy(SCPS) end
...
function SetupPlayers(pltab)
	...
	for _ = 1, pltab[1] do
		if #spctab < 1 then spctab = table.Copy(SCPS) end
	...
	for _, pl in pairs(allply) do
		for k, v in pairs(security) do
			if v.importancelevel > 1 then
				local can = true
				if v.customcheck ~= nil then if v.customcheck(pl) == false then can = false end end
				if can == true then
	...
	if GAS_AREAS then
		for k, v in pairs(GAS_AREAS) do
			if v.cont == true then
				...
				if (door1_tr1.Fraction == 1) and (door1_tr2.Fraction == 1) then ForceUse(button_doors_cont1, 1, 1) end
				...
				if (door2_tr1.Fraction == 1) and (door2_tr2.Fraction == 1) then ForceUse(button_doors_cont2, 1, 1) end
			end
		end
	end
end)
...
function UnlockContaminatedDoors()
	...
	for area_i, area in ipairs(GAS_AREAS) do
		if area.cont == true then table.remove(GAS_AREAS, area_i) end
	end
end
