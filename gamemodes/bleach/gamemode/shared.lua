...
if not ConVarExists("br_time_round") then
    CreateConVar("br_time_round", "720", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Set round time")
end
if not ConVarExists("br_time_postround") then
    CreateConVar("br_time_postround", "20", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Set postround time")
end
if not ConVarExists("br_time_gateb_open_min") then
    CreateConVar("br_time_gateb_open_min", "252", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Minimum time the gate b should open")
end
if not ConVarExists("br_time_gateb_open_max") then
    CreateConVar("br_time_gateb_open_max", "684", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Maximum time the gate b should open")
end
if not ConVarExists("br_time_ntfenter_delay_min") then
    CreateConVar("br_time_ntfenter_delay_min", "90", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Minimum time that NTF units will enter the facility")
end
if not ConVarExists("br_time_ntfenter_delay_max") then
    CreateConVar("br_time_ntfenter_delay_max", "180", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Maximum time that NTF units will enter the facility")
end
if not ConVarExists("br_time_blink") then
    CreateConVar("br_time_blink", "0.2", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Blink timer")
end
if not ConVarExists("br_time_blinkdelay") then
    CreateConVar("br_time_blinkdelay", "4", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Delay between blinks")
end
if not ConVarExists("br_specialround_percentage") then
    CreateConVar("br_specialround_percentage", "12", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Set percentage of special rounds")
end
if not ConVarExists("br_specialround_forcenext") then
    CreateConVar("br_specialround_forcenext", "none", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Force the next round to be a special round")
end
if not ConVarExists("br_scoreboardranks") then
    CreateConVar("br_scoreboardranks", "0", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "")
end
if not ConVarExists("br_defaultlanguage") then
    CreateConVar("br_defaultlanguage", "english", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "")
end
if not ConVarExists("br_expscale") then
    CreateConVar("br_expscale", "1", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "")
end
if not ConVarExists("br_force_roles") then
    CreateConVar("br_force_roles", "none", { FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY }, "Force roles for the next round")
end
...
function GM:ScalePlayerDamage(ply, hitgroup, dmginfo)
    ...
    if at ~= ply and at:IsPlayer() then
        if ply.UsingArmor ~= nil and dmginfo:IsDamageType(DMG_BULLET) then
