...
function OpenClassMenu()
	if IsValid(CLASS_MENU) then return end
	...
	class_modelpanel.Paint = function(self, w, h) draw.RoundedBox(0, 0, 0, w, h, Color(50, 50, 50)) end
	...
	for key, v in pairs(ALLCLASSES) do
		...
		for i, cls in ipairs(v.roles) do
			...
			if i ~= 1 then class_panel:DockMargin(0, 4, 0, 0) end
			...
			class_modelpanel:Dock(LEFT)
			class_modelpanel.Paint = function(self, w, h) draw.RoundedBox(0, 0, 0, w, h, Color(v.color.r - 25, v.color.g - 25, v.color.b - 25)) end
