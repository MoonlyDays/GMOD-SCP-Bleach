...
mapConfig = "mapconfigs/" .. game.GetMap() .. ".lua"
include(mapConfig)
include("hud.lua")
...
function DropCurrentVest()
    if LocalPlayer():Alive() and LocalPlayer():GTeam() ~= TEAM_SPECTATOR then
...
function LPoutside()
    ...
    if LocalPlayer():GetPos():WithinAABox(pos1, pos2) then
        return true
    end
    return false
end
...
function tick_flash()
    ...
    if f_started == true then
        if CurTime() > f_end then
            ...
            if f_brightness < 1 then
                f_brightness = f_brightness - f_fadein
            end
        end
    end
end
...
hook.Add("PreDrawHalos", "Halo173", function()
    ...
    if ply:GTeam() == TEAM_SCP then
        for i, v in pairs(players_to_highlight) do
            ...
            elseif IsValid(pl) == false or pl:Alive() == false or pl:GTeam() == TEAM_SCP or pl:GTeam() == TEAM_SPECTATOR then
                table.RemoveByValue(players_to_highlight, v)
                print("Removed " .. tostring(v[1]) .. " from players_to_highlight")
            end
        end
...
function HandleFootstepsCL()
    for k, v in pairs(player.GetAll()) do
        ...
        if v:GTeam() ~= TEAM_SPECTATOR and v:GetMoveType() ~= 8 and v:GetMoveType() ~= 10 and v:GetMoveType() ~= 0 and vel > 25 and v:IsOnGround() then
            if v.nextstep == nil then
                v.nextstep = 0
            end
            if v.nextstep > CurTime() then
                return true
            end
...
function GM:RenderScreenspaceEffects()
    ...
    if f_started == true and LocalPlayer():GTeam() ~= TEAM_SCP then
        brightness = brightness + f_brightness
    end
...
concommand.Add("br_requestescort", function()
    if not ((LocalPlayer():GTeam() == TEAM_GUARD or LocalPlayer():GTeam() == TEAM_CHAOS) or LocalPlayer():GTeam() == TEAM_CHAOS) then
        return
    end
