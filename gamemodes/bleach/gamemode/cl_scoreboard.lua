...
function ShowScoreBoard()
                ...
                for allpl_k, pl in pairs(player.GetAll()) do
                    if pl.GetNClass ~= nil then
                        if pl:Alive() and pl:GetNClass() == role.name then
                            table.ForceInsert(whole_list, pl)
                        end
                    end
                end
            end
    ...
    for k, v in pairs(to_remove) do
        for plist_k, plist in pairs(playerlist) do
            if v[2] == plist.name then
                table.RemoveByValue(plist.list, v[1])
            end
        end
    end
    ...
    for k, v in pairs(playerlist) do
        table.sort(v.list, function(a, b)
            return a:GetNLevel() > b:GetNLevel()
        end)
    end
    ...
    for key, tab in pairs(playerlist) do
        ...
        if #tab.list > 0 then
            ...
            for k, v in pairs(tab.list) do
                local nexp = 0
                if isfunction(v.GetNEXP) then
                    nexp = v:GetNEXP()
                end
                local nlvl = 0
                if isfunction(v.GetNLevel) then
                    nlvl = v:GetNLevel()
                end
                local panels = {
                    {
                ...
                scroll_panel.Paint = function(self, w, h)
                    ...
                    if (tab.color.r + tab.color.g + tab.color.b) > 500 then
                        tcolor2 = color_black
                    end
