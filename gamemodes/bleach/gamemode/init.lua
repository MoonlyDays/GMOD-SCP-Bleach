...
SCPS = {
	...
	{
		name = "SCP 966",
		func = function(pl) pl:SetSCP966() end
	}
}
...
function RequestOpenGateA(ply)
	...
	if not (ply:GTeam() == TEAM_GUARD or ply:GTeam() == TEAM_CHAOS) then return end
...
function SpawnNTFS()
	...
	for k, v in pairs(gteams.GetPlayers(TEAM_SPEC)) do
		if v.ActivePlayer == true then table.ForceInsert(activeplayers, v) end
	end
	...
	for _, rl in pairs(usablesupport) do
		for k, v in pairs(activeplayers) do
			if rl.role.level <= v:GetLevel() then
				local can = true
				if rl.role.customcheck ~= nil then if rl.role.customcheck(v) == false then can = false end end
				if can == true then table.ForceInsert(rl.list, v) end
			end
		end
	end
	...
	if usechaos == true then
		local chaosnum = 0
		for _, rl in pairs(usablesupport) do
			if rl.role.team == TEAM_CHAOS then chaosnum = chaosnum + #rl.list end
		end
...
function Use914(ent)
	...
	if b914status == 0 then return end
		...
		for k, v in pairs(POS_BUTTONS_TO_OPEN) do
			for k2, pos in pairs(v) do
				for k3, ent in pairs(ents.FindByClass(k)) do
					if ent:GetPos() == pos then ForceUse(ent, 1, 1) end
				end
			end
		end
	else
		for k, v in pairs(ents.FindByClass("func_door")) do
			if v:GetPos() == POS_173DOORS or v:GetPos() == POS_106DOORS then ForceUse(v, 1, 1) end
		end

		for k, v in pairs(ents.FindByClass("func_button")) do
			if v:GetPos() == POS_173BUTTON or v:GetPos() == POS_049BUTTON or v:GetPos() == POS_096BUTTON then ForceUse(v, 1, 1) end
		end

		for k, v in pairs(ents.FindByClass("func_rot_button")) do
			if v:GetPos() == POS_682BUTTON then ForceUse(v, 1, 1) end
		end
	end
end
...
function GetAlivePlayers()
	local plys = {}
	for k, v in pairs(player.GetAll()) do
		if v:GTeam() ~= TEAM_SPEC and v:Team() ~= TEAM_SPEC then if v:Alive() then table.ForceInsert(plys, v) end end
	end
	return plys
end
