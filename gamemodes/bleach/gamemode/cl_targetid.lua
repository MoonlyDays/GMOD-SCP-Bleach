...
function GM:HUDDrawTargetID()
	...
	if nextTGID <= CurTime() then
		table.ForceInsert(filterEnts, client)
		for k, v in pairs(player.GetAll()) do
			if IsValid(v:GetNWEntity("entity173")) then table.ForceInsert(filterEnts, v:GetNWEntity("entity173")) end
		end
	...
	if ply:IsPlayer() then
		...
		if ply:GetNClass() == ROLES.ROLE_SCP966 then
			...
			if client:GTeam() == TEAM_SCP then hide = false end
		...
		--end
		if ply:GTeam() == TEAM_CHAOS then clr = Color(29, 81, 56) end
		...
		if istable(frole) then
			if ply:GTeam() == TEAM_CHAOS then
				if LocalPlayer():GTeam() == TEAM_CHAOS or LocalPlayer():GTeam() == TEAM_CLASSD then
					...
					if frole.disguised_color ~= nil then clr = frole.disguised_color end
