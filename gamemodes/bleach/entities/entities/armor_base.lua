...
function ENT:Use(ply)
	if ply:GTeam() == TEAM_SPECTATOR or ply:GTeam() == TEAM_SCP or ply:Alive() == false then return end
	...
	if CLIENT then chat.AddText('You are now wearing an armor, type "dropvest" in the chat to drop it') end
...
function ENT:Draw()
	...
	if ply:GetPos():Distance(self:GetPos()) > 180 then return end
	if IsValid(self) then
		cam.Start2D()
		if DrawInfo ~= nil then DrawInfo(self:GetPos() + Vector(0, 0, 15), self.PrintName, Color(255, 255, 255)) end
		cam.End2D()
	end
end