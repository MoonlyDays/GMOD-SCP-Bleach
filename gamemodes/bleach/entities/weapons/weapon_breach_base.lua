...
function SWEP:AttachSilencer()
    if self.IsSilenced then
        ...
        if not (IsFirstTimePredicted() or game.SinglePlayer()) then
            return
        end
...
function SWEP:ShootBullet(damage, recoil, num_bullets, aimcone)
    ...
    if (not IsValid(self:GetOwner())) or (not self:GetOwner():Alive()) or self:GetOwner():IsNPC() then
        return
    end
