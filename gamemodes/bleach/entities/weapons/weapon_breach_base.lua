...
function SWEP:SecondaryAttack()
	...
	if self.HasScope then
		if self.IsScoping then
			...
			if CLIENT then self:EmitSound(self.ScopeSound) end
		else
			self.IsScoping = true
			if CLIENT then self:EmitSound(self.ScopeSound) end
		end
...
function SWEP:TranslateFOV(fov)
	if self.IsScoping == true then return self.ZoomFov end
	return fov
end
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		elseif self.DrawCustomCrosshair and self.HasScope and self.IsScoping == false and self.ZoomFov > self.MinFov then
			...
			scale = scale * (2 - math.Clamp((CurTime() - LastShootTime) * 7, 0.0, 1.0))
			surface.SetDrawColor(Color(0, 255, 0))
...
function SWEP:ShootBullet(damage, recoil, num_bullets, aimcone)
	local gap = aimcone + (neutralnum(self.Owner:GetVelocity():Length() / 25) / (self.IDK * 4))
	local bullet = {}
	...
	self:ShootEffects()
	if SERVER then bullet.Callback = function(attacker, tr, dmginfo) if attacker:IsPlayer() then if tr.Entity:GetClass() == "prop_vehicle_prisoner_pod" or tr.Entity:IsVehicle() then if tr.Entity:GetDriver() ~= NULL then tr.Entity:GetDriver():TakeDamageInfo(dmginfo) end end end end end
