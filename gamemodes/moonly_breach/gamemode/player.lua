...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
    --Camera fix from Breach 1.5
    if ply:GetNClass() ~= ROLE_SCP173 then
        ply:CreateRagdoll(ply, attacker, dmginfo:GetDamageType())
    end
    ply:AddDeaths(1)
end
...
function GM:PlayerCanPickupWeapon(ply, wep)
    ...
    if ply:GetNClass() ~= ROLE_SCP035 then
        if wep:GetClass() == "weapon_scp_035_deagle" then
            return false
        end
    end
...
function GM:PlayerUse(ply, ent)
    ...
    if button.clevel ~= nil then
        if ply:CLevel() < button.clevel then
            if button.usesounds == true then
                ply:EmitSound("breach/KeycardUse2.ogg")
            end
            ply.lastuse = CurTime() + 1
            ply:PrintMessage(HUD_PRINTCENTER, "You need to have " .. button.clevel .. " clearance level to open this door.")
            return false
        end
    end

    if button.canactivate ~= nil then
        local canactivate = button.canactivate(ply, ent)
        if not canactivate then
            if button.usesounds == true then
                ply:EmitSound("breach/KeycardUse2.ogg")
            end
            ply.lastuse = CurTime() + 1
            if button.customdenymsg then
                ply:PrintMessage(HUD_PRINTCENTER, button.customdenymsg)
            else
    ...
    if button.usesounds then
        ply:EmitSound("breach/KeycardUse1.ogg")
    end

    ply.lastuse = CurTime() + 1
    ply:PrintMessage(HUD_PRINTCENTER, "Access granted to " .. button["name"])
