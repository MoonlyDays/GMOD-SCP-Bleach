...
function mply:FindClosest(tab, num)
    ...
    table.sort(allradiuses, function(a, b)
        return a[1] < b[1]
    end)
    local rtab = {}
    for i = 1, num do
...
function mply:SpectatePlayerRight()
    ...
    if self:GetObserverMode() ~= OBS_MODE_IN_EYE and self:GetObserverMode() ~= OBS_MODE_CHASE then
        return
    end
    self:SetNoDraw(true)
...
function mply:ChangeSpecMode()
    ...
    if not (self:Team() == TEAM_SPECTATOR) then
        return
    end
    self:SetNoDraw(true)
