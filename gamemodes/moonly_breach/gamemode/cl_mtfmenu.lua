...
if not MTFMenuFrame then MTFMenuFrame = nil end
nextmenudelete = 0
showmenu = false
function GM:KeyPress(ply, key)
	if key == IN_ZOOM then OpenMenu() end
end

function GM:KeyRelease(ply, key)
	if key == IN_ZOOM then CloseMTFMenu() end
end

function CloseMTFMenu()
	if ispanel(MTFMenuFrame) then if MTFMenuFrame.Close then MTFMenuFrame:Close() end end
end

function OpenMenu()
	if IsValid(MTFMenuFrame) then return end
	local ply = LocalPlayer()
	if not (ply:Team() == TEAM_GUARD or ply:Team() == TEAM_CHAOS) then return end
