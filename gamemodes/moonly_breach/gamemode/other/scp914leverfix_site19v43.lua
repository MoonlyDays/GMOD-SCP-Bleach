...
print("Initializing 914fix for gm_site19_v43 by 'Xy' and edited by 'Dragon4k'.")
...
function Fix914()
	...
	if game.GetMap() != "gm_site19_v43" then return end
	for k,v in pairs(ents.GetAll()) do
		...
		if v and IsValid(v) and v:GetClass() == "func_button" and v.GetModel and v:GetModel() == "*159" then
			v:Fire("Unlock")
      ...
      --The 0 here just makes the timer trigger next frame, which, now that I think about it, is probably not needed
          ...
          --*221 is the brush model for the dial, it should also be unique to the dial
					if v.GetModel and (v:GetModel() == "*159" or v:GetModel() == "*164") then
						v:Fire("Unlock")
					end
				end
			end)
...
hook.Add("PlayerUse", "FixSCP914", function (ply, ent)
  ...
  if game.GetMap() == "gm_site19_v43" and IsValid(ent) and ent.GetModel and ent:GetModel() == "*159" then
