...
SWEP.SnapSound = Sound("breach/173/snap.wav")
SWEP.teams = {1}
...
function SWEP:Deploy()
	...
	self.Owner:SetWalkSpeed(550)
	self.Owner:SetRunSpeed(550)
	self.Owner:SetMaxSpeed(550)
end
	...
	if SERVER then
		...
		if ent:GetPos():Distance(self.Owner:GetPos()) < 150 then
			if ent:IsPlayer() then
				...
				if ent:GetClass() == "func_breakable" then ent:TakeDamage(1000, self.Owner, self.Owner) end
			end
		end
	end
end
...
function SWEP:SecondaryAttack()
	...
	if CLIENT then surface.PlaySound("breach/173/Horror2.ogg") end
	local findents = ents.FindInSphere(self.Owner:GetPos(), 600)
	local foundplayers = {}
	for k, v in pairs(findents) do
		if v:IsPlayer() then if not (v:Team() == TEAM_SCP or v:Team() == TEAM_SPECTATOR) then if v.usedeyedrops == false then table.ForceInsert(foundplayers, v) end end end
	end
	...
	if #foundplayers > 0 then
		...
		for k, v in pairs(foundplayers) do
			...
			v:SendLua('surface.PlaySound("breach/173/Horror2.ogg")')
...
function SWEP:DrawHUD()
	...
	local specialstatus = ""
	local showtext = ""
	local showtextlook = clang.nooneislooking
	local lookcolor = Color(0, 255, 0)
	local showcolor = Color(17, 145, 66)
	if self.NextSpecial > CurTime() then
		specialstatus = clang.readytousein .. math.Round(self.NextSpecial - CurTime()) .. clang.seconds
		showcolor = Color(145, 17, 62)
	else
		specialstatus = clang.readytouse
	end

	showtext = clang.special .. specialstatus
	if self.DrawRed < CurTime() then
		self.CColor = Color(255, 0, 0)
		showtextlook = clang.someoneislooking
