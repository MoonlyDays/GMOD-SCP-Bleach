...
sound.Add({
  ...
  sound = {"breach/weapons/scpcatj/catgun_fire01.wav", "breach/weapons/scpcatj/catgun_fire02.wav", "breach/weapons/scpcatj/catgun_fire03.wav", "breach/weapons/scpcatj/catgun_fire04.wav", "breach/weapons/scpcatj/catgun_fire05.wav", "breach/weapons/scpcatj/catgun_fire06.wav", "breach/weapons/scpcatj/catgun_fire07.wav", "breach/weapons/scpcatj/catgun_fire08.wav"}
})

sound.Add({
  name = "catgun.catgun_reload",
  channel = CHAN_ITEM,
  volume = 0.70,
  level = SNDLVL_NORM,
  sound = "breach/weapons/scpcatj/catgun_reload.wav"
})

sound.Add({
  name = "catgun.catgun_reload00",
  channel = CHAN_ITEM,
  volume = 0.70,
  level = SNDLVL_NORM,
  sound = "breach/weapons/scpcatj/catgun_reload00.wav"
})
...
sound.Add({
  name = "catgun.catgun_deploy",
  channel = CHAN_STATIC,
  volume = 0.70,
  level = SNDLVL_NORM,
  sound = "breach/weapons/scpcatj/catgun_deploy.wav"
})
...
function SWEP:Think()
  ...
  if CurTime() >= self:GetIronsightTime() then
    if switchmode then
      ...
      if sprinting then
        self:IronsightSwitchTo(false)
      elseif self.Owner:KeyPressed(IN_ATTACK2) then
        self:IronsightSwitchTo(not ironsight)
      end
    end
  end
...
function SWEP:Inspect()
  if self.Owner:IsNPC() then -- NPCs aren't supposed to reload it
    return
  end
...
function SWEP:DrawWeaponSelection(x, y, wide, tall, alpha)
  ...
  x = x + wide / 8
  y = y + tall / 3
  if texw == 64 then x = x + wide * .6 end
