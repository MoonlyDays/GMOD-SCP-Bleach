...
function SWEP:CheckSounds()
	if CLIENT then
		...
		elseif self.Channel == 3 then
			...
			local sound = "breach/radio/scpradio" .. self.LastSound
...
function SWEP:Think()
	...
	if self.Enabled then if self.IsLooping == false then if self.NextSoundCheck < CurTime() then self:CheckSounds() end end end
end
...
function SWEP:PrimaryAttack()
	...
	self:EmitSound("breach/radio/squelch.ogg")
	if self.Channel > 10 then self.Channel = 1 end
	self.IsLooping = false
	self:RemoveSounds()
	if self.Enabled then self:CheckSounds() end
...
function SWEP:SecondaryAttack()
	...
	if CLIENT then
		if self.Enabled then
			--self:CheckSounds()
			if IsValid(LocalPlayer().channel) then LocalPlayer().channel:SetVolume(1) end
		else
			self:StopSounds()
		end
	end
...
function SWEP:DrawHUD()
	...
	if self.Enabled then
		showtext = clang.radiochannel .. self.Channel
		showcolor = Color(0, 255, 0)
	else
		showtext = clang.radiodisabled
