...
function SWEP:Deploy()
	...
	if self.SoundDeploy ~= nil then self:EmitSound(self.SoundDeploy) end
	return true
end
...
function SWEP:Stab(atype, range)
	...
	if (not ent or not IsValid(ent)) and not ent:IsWorld() then
	...
	elseif ent:IsWorld() then
		...
		if atype == 2 then damagetodeal = self.Secondary.Damage end
		local cattack = true
...
function SWEP:StabDamage(entity, damage)
	...
	if SERVER then self.Owner:GetEyeTrace().Entity:TakeDamageInfo(dmginfo) end
end
